...
\end{equation}
\begin{equation}
H[w ,J]=\frac{1}{2 \beta } \int \int w (\br) u^{-1} (|\br-\br'|) w (\br') - n \ln Q[i w +J] ~ d\br d\br'
\end{equation}
...
\begin{equation}
\begin{aligned}
\langle \hat{ \rho } (\br) \rangle &=-n \frac{ \int e^{-(1/2\beta)\int \int wu^{-1}w+n\ln Q[iw]  d\br d\br'} \frac{\delta \ln Q[iw]}{\delta iw(\br)}\mathcal{D}w} {Z_{C}} \\&= \frac{i}{Z_{C}} \int e^{- (1/2 \beta ) \int \int w u^{-1} w d\br d\br'} \frac{ \delta }{\delta w (\br)} e^{n \ln Q[i w ]} ~\mathcal{D} w
\end{aligned}
\end{equation}
分部积分可得到\\
\begin{equation}
\begin{aligned}
\langle \hat{ \rho } (\br) \rangle &=\frac{i}{Z_{C}} \bigg[ e^{-A} e^{n \ln Q[i w ]}+\int e^{-A} e^{n \ln Q[i w ]} \frac{1}{2 \beta}(\int wu^{-1}d\br'+\int wu^{-1}d\br) \bigg ]\\&=\frac{i}{\beta}\int u^{-1} d \br' \frac{\int w \exp(-H[w])d\br'}{\int  \exp(-H[w])d\br'} \\&= \frac{i}{ \beta } \int u^{-1} (|\br-\br'|) d\br' \langle w (\br') \rangle 
\end{aligned}
\label{4.53}
\end{equation}
其中$A=-(1/2\beta)\int \int wu^{-1}w d\br d\br'$因此，粒子密度算子的“替代”表达式可以写为\\
\begin{equation}
...
“替代方法”对于高阶密度相关函数的估计特别方便。例如，两点函数可以写为\\
\begin{equation}
\begin{aligned}
\langle  \hat{ \rho } (\br) \hat{ \rho } (\br') \rangle &= \frac{1}{Z_{C}[J]} \frac{\delta ^{2}Z_{C}[J]}{\delta J(\br) \delta J(\br')} \bigg|_{J=0} \\&= -\frac{Z_0}{Z_{C}} \int  e^{-(1/2 \beta ) \int \int w u^{-1} w d\br d\br'} \frac{ \delta ^{2} \exp (n \ln Q[i w ])}{\delta w (\br) \delta w (\br')} ~\mathcal{D} w
\end{aligned}
\label{4.55}
\end{equation}
经过两次分部积分得出\\
\begin{equation}
\langle  \hat{ \rho } (\br) \hat{ \rho } (\br') \rangle = - \beta ^{-2} \int   u^{-1}(|\br-\br_{1}|) u^{-1}(|\br'-\br_{2}|) \langle w(\br_1) w(\br_2)\rangle d\br_{1} d\br_{2} + \beta ^{-1} u^{-1} (|\br-\br'|)
\label{4.56}
\end{equation}

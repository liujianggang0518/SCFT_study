...
\begin{aligned}
\label{3.79}
\sigma(\br;[w,\epsilon])&=\frac{1}{Q[w,\epsilon]} \frac{\int \hat{\sigma}_{\alpha \gamma} \exp [-\beta U_0 - \beta U_1 - \beta U_{el}]~d\br^{N+1}}{\int \exp [- \beta U_0 - \beta U_{el}]~d \br^{N+1}} \\
&= \frac{1}{Q[w,\epsilon]} \int \hat{\sigma}_{\alpha \gamma} \Psi(\br'-\br) \exp (-\beta U_1)~d \br^{N+1} \\
&= \frac{3k_{B}T}{b^{2}VQ} \int_{0}^{N} \delta(\br - \br(s)) \frac{d\br_{\alpha}(s)}{ds} \frac{d\br_{ \gamma}(s)}{ds}[ \Psi( \br_N- \br_{N-1})e^{-\Delta s w(r_{N})} \cdots \Psi(\br_1-\br_0)e^{-\Delta s w(r_0)} ]~ds \\
&=\frac{3k_{B}T}{b^{2}VQ (\Delta s)^2} \int_{0}^{N} q(\br + \Delta \br,N-s-\Delta s)q(\br,s) \Psi(\Delta \br,\br) \Delta \br_{\alpha} \Delta \br_{\gamma}~ds \\
&=\frac{3k_{B}T}{b^{2}VQ (\Delta s)^2} \int_{0}^{N} \frac{1}{2} \nabla \nabla q(\br,N-s) \frac{2b^4 (\Delta s)^2}{9}~ds \\
&=\frac{k_BTb^2}{3VQ[w,\epsilon]}\int_{0}^{N} q(\br,s;[w,\epsilon])~\nabla \nabla q(\br,N-s;[w,\epsilon])~ds
\end{aligned}
\end{equation}
其中将积分路径在$\br+\Delta \br$分解，链段从$s$离散到$s+\Delta s$，$q(\br + \Delta \br,N-s-\Delta s)$泰勒展开到一阶$\Delta s$,二阶$\Delta \br$。\\

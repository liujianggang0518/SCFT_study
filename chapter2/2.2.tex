...
\label{2.8}
\end{equation}
它是规范化的，所以$\int \int P_0(\br_0,\bm{n}^{N})d\br_0 d\bm{n}^{N}=1$，其中$\int d\bm{n}^{N}$表示单位球面上的$N$个积分。\\
...
\label{2.11}
\end{equation}
所以自由连接链模型的均方端到端向量完全符合理想链标度定律$R \sim bN^{\nu},\nu=1/2,N \rightarrow \infty $(2.1)：\\
\begin{equation}
...
\frac{1}{4 \pi} \int _{0}^{2 \pi} \int _{0}^{\pi} e^{ib|\bk|\bm{n} cos \theta}sin \theta \hat{p_0}(\bk,j-1) ~d \varphi d \theta \\&=\frac{1}{2} \int _{-1}^{1}cos b|\bk| \hat{p_0}(\bk,j-1)~dx \\&=j_0(b|\bk|)\hat{p_0}(\bk,j-1)
\label{2.20}
\end{aligned}
\end{equation}
...
\begin{equation}
F_0(\bbR)=-k_{B}T \ln p_0(\bbR,N)\approx \frac{3k_{B}T}{2Nb^2}|\bbR|^2
\label{2.24}
\end{equation}
